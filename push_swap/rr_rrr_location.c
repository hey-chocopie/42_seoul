...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/05 16:05:56 by hoylee            #+#    #+#             */
...
void	rrr_location(t_list **stack_a, t_list **stack_b, t_sd *s_data)
{
	...
	i = 0;
	while (i < s_data->rb_c && i < s_data->ra_c && (s_data->ra_c
					!= s_data->ca && s_data->rb_c != s_data->cb))
	{
...
void	rr_location(t_list **stack_a, t_list **stack_b, t_sd *s_data)
{
	...
	i = 0;
	while (i < s_data->cb - s_data->rb_c && i < s_data->ca - s_data->ra_c
			&& (s_data->ra_c != s_data->ca && s_data->rb_c != s_data->cb))
	{
