...
/*   Updated: 2020/11/24 13:45:50 by hoylee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "key_macos.h"
...
void	ft_set_fc(t_info *info, float *fl_step_xy, int y, float *floor_xy)
{
		...
		fl_step_xy[0]
		 = rowDistance * (raydir_xx[1] - raydir_xx[0]) / info->width;
		fl_step_xy[1]
		 = rowDistance * (raydir_yy[1] - raydir_yy[0]) / info->width;
...
void	ft_colr_fc(t_info *info,  float *floor_xy,float *fl_step_xy, int y)
{
	...
	while(x < info->width)
	{
		tx = (int)(info->texture_x_size * (floor_xy[0] - (int)(floor_xy[0]))) & (info->texture_x_size - 1);
		ty = (int)(info->texture_y_size * (floor_xy[1] - (int)(floor_xy[1]))) & (texHeight - 1);
