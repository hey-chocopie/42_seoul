...
void Form::check_grade_and_signed(Bureaucrat const & rhd) const
{
	if(getSigned() == false)
...
{
	std::cout << "[Name] :"  << rhd.getName( ) << " [Sigend] : " <<  rhd.getSigned( ) << " [SignGrade] : " << rhd.getSignGrade( ) << " [ExecuteGrade] : " << rhd.getExecuteGrade() << std::endl;
	return (os);
}
